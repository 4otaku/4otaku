...
function getElFromArrByProp(arr,prop,str) {
  for (var i in arr) {
    if (arr.hasOwnProperty(i) && arr[i][prop] == str)
      return arr[i];
  }
  return null;
}
...
function RosterOpenGroupchat(aJid,nick,pass) {
  ...
  frames['chatW'].location.replace("groupchat.html?ver=2&jid="+encodeURI(aJid)+"&nick="+encodeURI(nick)+"&pass="+encodeURI(pass));
...
function RosterUpdateGroupForUser(user) {
  for (var j in user.groups) {
    if (user.groups.hasOwnProperty(j)) {
...
function printRoster() {
  ...
  for (var i=0; i<this.groups.length; i++) {
    ...
    for (var j=0; j<this.groups[i].users.length; j++) {
      ...
      rosterHTML += "<div id=\"rosterUser_"+user.name+"\" class=\"rosterUser\" onMouseOver=\"highlight(this);\" onMouseOut=\"unhighlight(this);\" username=\""+user.name+"\" title=\"Написать в личку\"><span class=\"user_"+user.status+"\">"+user.name+"</span>";
      if (user.statusMsg)
