...
class output__news extends engine
{
	...
	function get_data() {
		global $url; global $db; global $def; global $sets; global $side_modules;
		if ($url[2] && $url[2] != 'page') {
			...
			$return['navi']['start'] = max($return['navi']['curr']-5,2);
			$return['navi']['last'] = ceil($db->sql('select count(id) from news',2,'count(id)')/$sets['pp']['news']);		
	...
	function get_news($limit, $area) {
		...
		$return = $db->sql('select * from news '.$area.' order by sortdate desc limit '.$limit);
		if (is_array($return)) {
