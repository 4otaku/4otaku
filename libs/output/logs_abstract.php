...
abstract class output__logs_abstract extends engine
{
	public static $room_ids = array();
	protected $start_year = 1970;
	protected $start_month = 1;
	protected $start_day = 1;
	public static $cache_key = 0;
	...
	function get_data() {
		...
		if (!$this->nocache) {
			$return['logs'] = base64_decode(obj::db()->sql('select cache from logs where (year='.$url[2].' and month ='.$url[3].' and day='.$url[4].' and cache_key ='.$this->cache_key.')',2,'cache'));
		}
		if (!$return['logs']) {
		...
		if ($yesterday > $start_time) {
			$return['navi']['yesterday']= array(
				'url' => '/'.$base.'/'.date("Y",$yesterday).'/'.date("n",$yesterday).'/'.date("j",$yesterday).'/',
		...
		if (is_array($logs)) {
			...
			foreach ($logs as &$log) {
				...
				$cache .= '<div class="logs-'.$key.'" id="time-'.date('G:i:s',$log['logTime']/1000).'.'.($log['logTime']%1000).'">
								<a href="/'.$base.'/'.$year.'/'.$month.'/'.$day.'#time-'.date('G:i:s',$log['logTime']/1000).'.'.($log['logTime']%1000).'">
			...
			if (!obj::db()->sql('select id from logs where (year='.$year.' and month ='.$month.' and day='.$day.')',2,'id'))
				obj::db()->insert('logs',array(base64_encode($cache),$year,$month,$day,$this->cache_key));
		}
	}
	...
	function search ($query, $page) {
		...
		$select_query = ' MATCH (text) AGAINST ("'.$fixed_query.'" IN BOOLEAN MODE) and cache_key = ' . $this->cache_key;
		...
		if (!empty($days)) {
			...
			$return['navi']['last'] = ceil(obj::db()->sql('select count(distinct(date)) from raw_logs WHERE MATCH (text) AGAINST ("'.$fixed_query.'" IN BOOLEAN MODE) and cache_key = ' . $this->cache_key,2)/$perpage);
		} else {
