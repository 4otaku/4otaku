...
class output__logs extends engine
{
	...
	function get_data() {
		...
		if (!$return['logs']) {
			...
			$return['logs'] = obj::db('chat')->sql('select nickname, logTime, body from ofMucConversationLog where ((roomID = 37 or roomID = 2 or roomID = 1) and cast(logTime as unsigned) > '.$today.' and cast(logTime as unsigned) < '.($today + 86400000).') order by logTime');
		...
		if ($yesterday > mktime(0,0,0,$start[1],$start[2],$start[0])) {
			...
			$return['nologs'] = "Логов раньше, чем за 15-ое февраля 2010 попросту нет, комната появилась в тот день.";
