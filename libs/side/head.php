...
class side__head {
    ...
    /* global settings array, for int usage */
    public $def = array();
    ...
    function title_art() {
        ...
        elseif ($url[2] == 'mixed') {
            ...
            if (!is_numeric($url[3]))
                $return = '. Просмотр списка групп.';
            else
                $return = '. Группа: ' . $data['main']['pool']['name'];
        }
        else {
            $types = $this->types;
    ...
    function title_post() {
        ...
        if (is_numeric($url[2])) {
            ...
            if (is_array($data['main']['post'][0]['links']))
                foreach ($data['main']['post'][0]['links'] as $link)
                    if (is_array($link['search']))
                        foreach ($link['search'] as $alias)
                            $download[] = $alias;
                    else
                        foreach ($link['alias'] as $alias)
                            if (!is_numeric($alias))
                                $download[] = $alias;
            if ($download)
                $return .= '. Скачать с ' . implode(', ', array_unique($download));
        }
        elseif ($url[2] == 'mixed') {
    ...
    function title_order() {
        ...
        global $url;
        if (is_numeric($url[2]))
            return $this->def['site']['short_name'] . ' Заказ: ' . $data['main']['order_single']['title'];
