		...
		onComplete: function(id, file, response) {
			...
			if (response['error'] == 'filetype') {$('#error').html('<b>Ошибка! Выбранный вами файл не является картинкой.</b>');}
				...
				if ($("input."+response['md5']).length < 1) {
					...
					$('#transparent td').append('<div style="background-image: url('+response['image']+');"><img class="cancel" src="'+window.config.image_dir+'/cancel.png"><input type="hidden" name="images[]" value="'+response['data']+'" class="'+response['md5']+'"></div>');
