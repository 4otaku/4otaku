	...
	board_upload = new qq.FileUploader({
		...
		onComplete: function(id, file, response) {
			...
			if (response['error'] == 'filetype') {$('#error').html('<b>Ошибка! Выбранный вами файл не является картинкой.</b>');}
				...
				$('#transparent td').html('<div style="background-image: url('+response['image']+');"><img class="cancel" src="'+window.config.image_dir+'/cancel.png"><input type="hidden" name="image" value="'+response['data']+'"></div>');
