...
<hr />
<a href="<?=SITE_DIR.'/video'?>/workshop/">
...
<ul class="car-monthlisting">
	<?
		if (is_array($data['main']['tags'])) foreach ($data['main']['tags'] as $key => $item) {
			?>
				<li>
					<a href="<?=SITE_DIR.'/art'?>/tag/<?=$item['data1'];?>/"><?=$item['data1'];?></a> 
					...
					<? } else { ?>
						<a href="<?=SITE_DIR.'/admin'?>/tags/search/<?=$item['data1'];?>/">
							Перейти к тегам
						</a>.
					<? } ?> 
		...
		if (is_array($data['main']['comment'])) foreach ($data['main']['comment'] as $item) {
			?>
				<div class="mini-shell">
					...
					<a href="<?=SITE_DIR?>/<?=($item['place'] == 'orders' ? 'order' : $item['place']);?>/<?=$item['post_id'];?>/comments/all#comment-<?=$item['id'];?>">
