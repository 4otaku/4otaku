<div class="shell">
	...
	<? if (file_exists('/var/www/nameless/data/www/w8m.4otaku.ru/image/'.$data['main']['pool']['md5'].'.zip')) { ?>
		<a href="<?=$def['site']['dir']?>/art/download/<?=_base64_encode(hex2bin($data['main']['pool']['md5']),true);?>" target="_blank">Скачать их одним архивом.</a> (~<?=ceil($data['main']['pool']['filesize']/1024/1024);?> мб)
	<? } ?>
	<br />	
