<div class="mini-shell">
	Вы просматриваете CG из игры "<a href="<?=$def['site']['dir']?>/art/cg_packs/<?=$data['main']['gallery']['id'];?>/"><?=$data['main']['gallery']['name'];?></a>". 
	<? if (file_exists('/var/www/nameless/data/www/w8m.4otaku.ru/image/'.$data['main']['gallery']['md5'].'.zip')) { ?>
		<a href="<?=$def['site']['dir']?>/art/download/<?=_base64_encode(hex2bin($data['main']['gallery']['md5']),true);?>" target="_blank">Скачать их одним архивом.</a> (~<?=ceil($data['main']['gallery']['filesize']/1024/1024);?> мб)
	<? } ?><br />
	Имя файла: <a href="<?=$def['site']['dir']?>/art/download/<?=_base64_encode(hex2bin($data['main']['art']['md5']),true);?>" target="_blank"><?=$data['main']['art']['filename'];?></a> 
</div>
<br />
<div class="post">
	<div class="innerwrap">
				...
				<td class="booru_main"> 
					<? if ($data['main']['art']['resized'] && $sets['art']['resized']) { ?>
						...
						<div class="booru_img image booru" rel="resized">
							<a href="<?=$def['site']['dir']?>/art/download/<?=_base64_encode(hex2bin($data['main']['art']['md5']),true);?>" target="_blank">
