<?
	...
	foreach ($lang['link'] as $key => $one) {
		?>
			<a href="<?=$def['site']['dir']?>/art/<?=($key == $def['area'][0] ? "" : $key."/");?>"<?=($key == $url[2] || ($key == $url['area'] && $url[2] != 'pool' && substr($url[2],0,3) != 'cg_') || substr($url[2],0,3) == 'cg_' && $key == 'cg_packs' ? ' class="plaintext"' : '');?>>
				<?=$one;?>
			</a>
			<br />				
		<?
