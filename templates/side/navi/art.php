<?
	...
	foreach ($lang['link'] as $key => $one) {
		?>
			<a href="/art/<?=($key == $def['area'][0] ? "" : $key."/");?>"<?=($key == $url[2] || ($key == $url['area'] && $url[2] != 'pool' && substr($url[2],0,3) != 'cg_') || substr($url[2],0,3) == 'cg_' && $key == 'cg_packs' ? ' class="plaintext"' : '');?>>
				<?=$one;?>
			</a>
			<br />				
		<?
	...
	if (is_array($data['main']['display']) && in_array('booru_page',$data['main']['display']) && $url[2] != 'page' && $url[3] && $url['area'] == 'main' && !$error) {
		?>
			<br />
			<a href="/art/slideshow/<?=$url[2];?>/<?=$url[3];?>#<?=(1 + ($data['main']['navi']['curr'] - 1)*$sets['pp']['art']);?>" target="_blank">
				Запустить слайдшоу
			</a>
		<?
	}
