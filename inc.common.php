...
function __autoload($class_name) {
	$class = SITE_FDIR._SL.'libs'.SL.str_replace('__',SL,$class_name) . '.php';
#	$class = 'libs'.SL.str_replace('__',SL,$class_name) . '.php';
	if (file_exists($class)) include_once $class;
	else {
		if (_INDEX_) {
			include_once SITE_FDIR._SL.'templates'.SL.'404'.SL.'fatal.php';
			ob_end_flush();
		}
		die;
	}
...
include_once SITE_FDIR._SL.'engine'.SL.'config.php';
